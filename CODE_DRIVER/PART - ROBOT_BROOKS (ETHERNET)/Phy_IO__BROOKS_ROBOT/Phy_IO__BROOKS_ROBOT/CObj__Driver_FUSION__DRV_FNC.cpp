...
::__Read__DIGITAL(const CString& var_name,const CDS_IO__CHANNEL_INFO& io_info, CString& read_data,int& item_index)
{
	...
	if(diCH__ARM_A_WFR->Check__VARIABLE_NAME(var_name) > 0)
	{
		...
		return Drv__SEND_RECV(var_name, str_cmmd, _TO__QUERY, TRUE, l_rsp,l_err) ;
	}
	// eRB1.VacSnsA   (ON OFF)
...
::__Read__STRING(const CString& var_name, const CDS_IO__CHANNEL_INFO& io_info, CString& read_data)
{
	...
	if(siCH__RBx_GetMap->Check__VARIABLE_NAME(var_name) > 0)
	{
		str_cmmd.Format("atm1 GetLocationStates");

		int r_flag = Drv__SEND_RECV(var_name, str_cmmd, to_sec, TRUE, l_rsp,l_err) ;
		if(r_flag < 0)
		{
			read_data = "Error";
			return -1;
	...
	if(siCH__LPx_GetMap->Check__VARIABLE_NAME(var_name) > 0)
	{
		str_cmmd.Format("%s getlocationstates",	sDRV__STN_NAME);
	...
	if(siCH__ALx_RESULT->Check__VARIABLE_NAME(var_name) > 0)
	{
		...
		if(err_count > 0)		read_data = "Error";
		else					read_data = "OK";
...
::__Write__ANALOG(const CString& var_name, const CDS_IO__CHANNEL_INFO& io_info, const double set_data)
{
	...
	if(aoCH__ALIGN_SLOT->Check__VARIABLE_NAME(var_name) > 0)
	{
		iDRV__ANGLE_SLOT = (int) set_data;
		return 1;  
	}
...
::__Write__DIGITAL(const CString& var_name,const CDS_IO__CHANNEL_INFO& io_info, const CString& set_data,const int item_index)
{
	...
	//  PrepPick  PrepPlace  Pick  Place  GetLocSte  Home  Reset  MapStn 
	if(doCH__RBx_CMMD->Check__VARIABLE_NAME(var_name) > 0)
	{
		...
		{
			sCH__RBx_CMMD_STATE->Set__DATA("Cmmd Error");
			return -1;
		}

		int r_flag = Drv__SEND_CMMD(var_name, str_cmmd, TRUE) ;
		if(r_flag < 0)
		{
			sCH__RBx_CMMD_STATE->Set__DATA("Aborted");
			return -1;
		}
		return 1;   
	}

	//  InitAlgn  ALAlign  ALHome  ALLoadPP
	if(doCH__ALx_CMMD->Check__VARIABLE_NAME(var_name) > 0)
	{
		...
		if(set_data.CompareNoCase("InitAlgn") == 0) 
		{
		...
		{
			str_cmmd.Format("algn1 align atm1 %1d %1d", iDRV__ANGLE_SLOT, (int) dDRV__RB_ANGLE_DEG);
		}
		...
		{
			str_cmmd.Format("algn1 Scan atm1 %1d", iDRV__ANGLE_SLOT);
		}
		else if(set_data.CompareNoCase("ALLoadPP") == 0) 
		{
			str_cmmd.Format("algn1 align atm1 %1d %d", iDRV__ANGLE_SLOT, (int) dDRV__RB_ANGLE_DEG);
		}
		else
		{
			sCH__ALx_CMMD_STATE->Set__DATA("Cmmd Error");
			return -1;
		}
		...
		if(r_flag < 0)
		{
			sCH__ALx_CMMD_STATE->Set__DATA("Aborted");
			return -1;
		}
		return 1;   
	}
	//  ALResult
	if(doCH__ALx_REPORT->Check__VARIABLE_NAME(var_name) > 0)
	{
		double to_sec = _TO__QUERY;

		if(set_data.CompareNoCase("ALResult") == 0) 
		{
			sCH__AL_GetInfo_Update_Req->Set__DATA(STR__YES);
	...
	*/
	...
	if(doCH__SYSTEM_TRACKING_MATERIAL->Check__VARIABLE_NAME(var_name) > 0)
	{
		...
		int r_flag = Drv__SEND_RECV(var_name, str_cmmd, _TO__QUERY, TRUE, l_rsp,l_err) ;
...
::__Write__STRING(const CString& var_name, const CDS_IO__CHANNEL_INFO& io_info, const CString& set_data)
{
	...
	if(soCH__CHECK_CMMD->Check__VARIABLE_NAME(var_name) > 0)
	{
		...
		if(Drv__SEND_RECV(var_name, set_data, to_sec, TRUE, l_rsp,l_err) < 0)
		{
			return -1;
		}
		return 1;
