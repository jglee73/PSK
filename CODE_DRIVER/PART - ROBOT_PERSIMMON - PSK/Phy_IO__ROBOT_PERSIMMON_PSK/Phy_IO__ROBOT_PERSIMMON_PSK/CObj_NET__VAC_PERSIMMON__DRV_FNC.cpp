	...
	|| (siCH__RB_STS_READ_MANUAL->Check__VARIABLE_NAME(var_name) > 0))
	{
		...
		if(r_flag > 0)
		{
			if(l_rsp.GetSize() == 6)
			{
				...
				if(dCH__CFG_ROBOT_DRV_WAFER_CHECK->Check__DATA(STR__YES) > 0)
				{
					// Arm_A.Load
					{
					...
					// Arm_B.Load
					{
						...
						if(arm_b_data.CompareNoCase("Y") == 0)			str_load = STR__ON;
	...
	if(doCH__COMMAND->Check__VARIABLE_NAME(var_name) > 0)
	{
		...
		*/
		...
		else if(set_data.CompareNoCase(_Cmmd__Wfr_Cen_Data) == 0)
		{
			...
			active__act_cmmd = false;
			to_sec = 1;	// sec
		}
		else if(set_data.CompareNoCase(_Cmmd__Set_Speed) == 0)
		{
			double cfg__speed_per = aCH__CFG_ROBOT_SPEED_PERCENT->Get__VALUE();

			str_cmmd.Format("native setspeedscale %0.1f", cfg__speed_per / 100.0);
		...
		int r_flag = Proc__CMMD_RSP(active__act_cmmd, var_name, str_cmmd, to_sec, TRUE, l_rsp,l_err) ;
		if(r_flag < 0)		return -1;
		...
		if(l_rsp.GetSize() > 0)
		{
			...
			{
				sCH__DRV_INFO_ROBOT_SPEED->Set__DATA(rsp_data);
			}
		}
