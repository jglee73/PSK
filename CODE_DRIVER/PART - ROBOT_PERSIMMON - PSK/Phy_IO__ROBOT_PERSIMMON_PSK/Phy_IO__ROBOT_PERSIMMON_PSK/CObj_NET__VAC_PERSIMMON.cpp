...
int CObj_NET__VAC_PERSIMMON::__DEFINE__VARIABLE_STD(p_variable)
{
	...
	// DRV_INFO ...
	{
		...
		//
		str_name = "DRV.INFO.ROBOT.SPEED";
		STD__ADD_STRING(str_name);
		LINK__VAR_STRING_CTRL(sCH__DRV_INFO_ROBOT_SPEED, str_name);
	}
		...
		for(i=0; i<CFG_LLx__SIZE; i++)
		{
			...
			for(int k=0; k<CFG_LLx__SLOT_SIZE; k++)
			{
				...
				// STN NUM ...
				{
					...
					LINK__VAR_ANALOG_CTRL(aCH__OTR_IN_CFG__LLx_STN_NUM_X[i][k], str_name);			
				}
				...
				{	
					...
					LINK__VAR_ANALOG_CTRL(aCH__OTR_IN_CFG_LLx_CPTR_SNS_NUM_X[i][k], str_name);
				}
			}
		}
	}
	...
	// CFG.ROBOT ...
	{
		...
		STD__ADD_ANALOG_WITH_X_OPTION(str_name, "%", 0, 10, 90, "");
		LINK__VAR_ANALOG_CTRL(aCH__CFG_ROBOT_SPEED_PERCENT, str_name);

		str_name = "CFG.ROBOT.DRV.WAFER.CHECK";
		STD__ADD_DIGITAL_WITH_X_OPTION(str_name, "YES NO", "");
		LINK__VAR_DIGITAL_CTRL(dCH__CFG_ROBOT_DRV_WAFER_CHECK, str_name);
	}
...
LOOP_RETRY:
	...
	// Material.Check ...
	{
		if(sCH__MON_ROBOT_ARM_A_LOAD->Check__DATA(STR__ON) > 0)
		{
		...
		if(sCH__MON_ROBOT_ARM_B_LOAD->Check__DATA(STR__ON) > 0)
		{
			if(dCH__MON_ARM_B_MATERIAL_STATUS->Check__DATA(STR__NONE) > 0)
				dCH__MON_ARM_B_MATERIAL_STATUS->Set__DATA(STR__EXIST);
		}
		else
		{
			dCH__MON_ARM_B_MATERIAL_STATUS->Set__DATA(STR__NONE);
		}
	}
