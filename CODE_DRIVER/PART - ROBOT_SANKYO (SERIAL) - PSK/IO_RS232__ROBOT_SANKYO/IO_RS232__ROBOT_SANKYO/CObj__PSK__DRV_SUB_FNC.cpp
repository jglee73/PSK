...
int  CObj__PSK::Update__ROBOT_STATE(const CString& str_data)
{
	...
	for(i=0; i<i_limit; i++)
	{
		...
		if(i == 9)			// Aligner 동작중     [09] : 
		{
				 if(ch == '1')			str__al_sts = STR__ON;
			else if(ch == '0')			str__al_sts = STR__OFF;
			else						str__al_sts = STR__UNKNOWN;
	...
	if(active__recv_ok)
	{
		...
		sCH__MON_AL_BUSY_STATE->Set__DATA(str__al_sts);
	...
	else
	{
		...
		sCH__MON_AL_BUSY_STATE->Set__DATA(ch_data);
...
int  CObj__PSK::Update__AL_MEASUREMENT(const CString& str_data)
{
	...
	for(i=0; i<i_limit; i++)
	{
		...
		{
			str__x_err += ch;			// 0.1 mm
			continue;;
		}
		if((i >= 6) && (i <= 9))		// X 방향 틀어짐     [06 ~ 09] : 
		{
			str__y_err += ch;			// 0.1 mm
			continue;;
		}
		if((i >= 10) && (i <= 14))		// Notch 각도     [10 ~ 14] : 
		{
			str__notch_angle += ch;		// 0.1 degree
			continue;;
		}
	}
...
void CObj__PSK::Update__WAFER_INFO()
{
	// ARM_A.STATE ...
	{
		CString wafer_state = sCH__MON_A_ARM_GRIPPER_STATE->Get__STRING();

			 if(wafer_state.CompareNoCase(STR__WAFER) == 0)			dCH__MON_ARM_A_MATERIAL_STATUS->Set__DATA(STR__EXIST);
		else if(wafer_state.CompareNoCase(STR__NONE)  == 0)			dCH__MON_ARM_A_MATERIAL_STATUS->Set__DATA(STR__NONE);
		else														dCH__MON_ARM_A_MATERIAL_STATUS->Set__DATA(STR__UNKNOWN);
	}

	// ARM_B.STATE ...
	{
		CString wafer_state = sCH__MON_B_ARM_GRIPPER_STATE->Get__STRING();

			 if(wafer_state.CompareNoCase(STR__WAFER) == 0)			dCH__MON_ARM_B_MATERIAL_STATUS->Set__DATA(STR__EXIST);
		else if(wafer_state.CompareNoCase(STR__NONE)  == 0)			dCH__MON_ARM_B_MATERIAL_STATUS->Set__DATA(STR__NONE);
		else														dCH__MON_ARM_B_MATERIAL_STATUS->Set__DATA(STR__UNKNOWN);
	}
}
