...
void CReportCtrl::SetParent(CWnd * pWnd)
{
	m_pParentWnd = pWnd;
}
...
{
	printf("CReportCtrl::OnColumnclick() - Start ! \n");

	if(m_bNoSort)
	{
		printf("CReportCtrl::OnColumnclick() - Error ! \n");
		return;
	}
	...
	*pResult = 0;
	...
	printf("CReportCtrl::OnColumnclick() - OK ! \n");
}
...
//	if(idx > -1)
//	{
//		SetCheck(idx, 1-GetCheck(idx));
//
//		WPARAM wParam;
//		wParam = WM_LBUTTONUP;
//		wParam <<= 16;
//		wParam |= this->GetDlgCtrlID();
//		m_pParentWnd->SendMessage(WM_COMMAND, wParam, (LPARAM)this->GetSafeHwnd());
//	}
...
int CReportCtrl::InsertItem(int nItem, 
							LPCSTR lpText, 
							COLORREF textColor, 
							COLORREF backColor,
							BOOL redraw)
{
	...
	if (IDX >= 0)	_AllocItemMemory(IDX);
	
	SetItemTextColor(nItem, 0, textColor, redraw);
	SetItemBkColor(nItem, 0, backColor, redraw);
...
BOOL CReportCtrl::SetItemText(int nItem, 
							  int nSubItem, 
							  LPCSTR lpText, 
							  COLORREF textColor, 
							  COLORREF backColor,
							  BOOL redraw)
{
	SetItemText(nItem, nSubItem, lpText);
	
	SetItemTextColor(nItem, nSubItem, textColor, redraw);
	SetItemBkColor(nItem, nSubItem, backColor, redraw);
...
void CReportCtrl::OnItemchanged(NMHDR* pNMHDR, LRESULT* pResult) 
{
	NM_LISTVIEW* pNMListView = (NM_LISTVIEW*)pNMHDR;
