...
int CObj__PSK::__DEFINE__CONTROL_MODE(obj,l_mode)
{
	...
	// ...
	{
		...
		ADD__CTRL_VAR(sMODE__ERROR_RESET, "ERROR.RESET");
		...
		ADD__CTRL_VAR(sMODE__AL_CHUCK_ON,  "AL.CHUCK_ON");
		ADD__CTRL_VAR(sMODE__AL_CHUCK_OFF, "AL_CHUCK_OFF");
	}
...
int CObj__PSK::__DEFINE__VARIABLE_STD(p_variable)
{
	...
	// CFG.ROBOT ...
	{
		str_name = "CFG.ROBOT.SPEED.PERCENT";
		STD__ADD_ANALOG_WITH_X_OPTION(str_name, "%", 0, 1, 99, "");
		LINK__VAR_ANALOG_CTRL(aCH__CFG_ROBOT_SPEED_PERCENT, str_name);
	}
	...
	// PARA : RB ...
	{
		...
		str_name = "PARA.RB.STAGE_CODE";
		STD__ADD_ANALOG(str_name, "pos", 0, 0, 1);
		LINK__VAR_ANALOG_CTRL(aCH__PARA_RB_STAGE_CODE, str_name);
	...
	// PARA : AL ...
	{
		...
		STD__ADD_ANALOG(str_name, "deg", 1, 0, 360);
	...
	// MON ...
	{
		...
		str_name = "MON.AL.BUSY.STATE";
		STD__ADD_STRING(str_name);
		LINK__VAR_STRING_CTRL(sCH__MON_AL_BUSY_STATE, str_name);
	...
	// CONFIG CHANNEL ...
	{
		...
		str_name = "CFG.ALIGN.MEASUREMENT.USE";
		STD__ADD_DIGITAL_WITH_X_OPTION(str_name, "NO YES", "");
		LINK__VAR_DIGITAL_CTRL(dCH__CFG_ALIGN_MEASUREMENT_USE, str_name);
...
int CObj__PSK::__CALL__CONTROL_MODE(mode, p_debug, p_variable, p_alarm)
{
	...
	if(dCH__CFG_ACTION_CONFIRM_FLAG->Check__DATA(STR__ENABLE) > 0)
	{
		...
		Get__CFG_STN_NUM(para__stn_name,para__stn_slot, stage_code,stn_id,stn_slot);
		...
		str_msg += str_bff;
		str_bff.Format("Para.Station_Name : [%s] \n", para__stn_name);
		str_msg += str_bff;
		str_bff.Format("Para.Station_Slot : [%s] \n", para__stn_slot);
		str_msg += str_bff;
		str_msg += "\n";

		str_bff.Format("Code.Stage_Point  : [%s] \n",  stage_code);
		str_msg += str_bff;
		str_bff.Format("Code.Station_ID   : [%s] \n",  stn_id);
		str_msg += str_bff;
		str_bff.Format("Code.Station_Slot : [%s] \n", stn_slot);
		str_msg += str_bff;
	...
	if(seq_flag > 0)
	{
	    ...
	    ELSE_IF__CTRL_MODE(sMODE__HOME)					flag = Call__HOME(p_variable,p_alarm, false);

		ELSE_IF__CTRL_MODE(sMODE__ERROR_RESET)			flag = Call__ERROR_RESET(p_variable,p_alarm);
		...
		ELSE_IF__CTRL_MODE(sMODE__AL_CHUCK_ON)			flag = Call__AL_CHUCK_ON(p_variable,p_alarm);
		ELSE_IF__CTRL_MODE(sMODE__AL_CHUCK_OFF)			flag = Call__AL_CHUCK_OFF(p_variable,p_alarm);
	}
