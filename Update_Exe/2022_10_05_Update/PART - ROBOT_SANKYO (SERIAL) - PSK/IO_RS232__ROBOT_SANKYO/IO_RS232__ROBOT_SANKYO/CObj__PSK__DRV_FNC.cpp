...
::__Write__DIGITAL(const CString& var_name,const CDS_IO__CHANNEL_INFO& io_info, const CString& set_data,const int item_index)
{
	...
	if(doCH__RB_COMMAND->Check__VARIABLE_NAME(var_name) > 0)
	{
		...
		str_stage.Format("%02d", (int) aCH__PARA_RB_STAGE_CODE->Get__VALUE() );
		...
		{
			str__cmmd = "01";
			str__para.Format("0000000000%s", str_speed);
		}
		else if(set_data.CompareNoCase("PICK") == 0)
		{
			str__cmmd = "0204";
			str__para.Format("%s%s%s%s%s", str_stage, str_pos,str_slot,str_arm, str_speed);
		}
		...
		{
			str__cmmd = "0206";
			str__para.Format("000000%s%s", str_arm, str_speed);
		}
	...
	if(doCH__AL_COMMAND->Check__VARIABLE_NAME(var_name) > 0)
	{
		...
		else if(set_data.CompareNoCase(_AL_MODE__MEASURE_START) == 0)
		{
			int para__notch_angle = (int) (aCH__PARA_AL_NOTCH_ANGLE->Get__VALUE() * 100.0);			
			str_data.Format("%05d", para__notch_angle);

			str__cmmd = "1301";
			str__para.Format("0%s0000", str_data);						
		}
		else if(set_data.CompareNoCase(_AL_MODE__CHUCK_ON) == 0)
		{
			str__cmmd = "1302";		

			str__para = str__angle_deg;
			str__para += "000000"; // 221004_KMS:220909 MManual_Adapt
		...
		else if(set_data.CompareNoCase(_AL_MODE__ROT_ABSOLUTE) == 0)
		{
