...
#define _DRV_CMMD__SET_CLAER_LATCHED_FAULTS					119 // KMS:221026, Latch Fault Reset
...
::__Read__STRING(const CString& var_name, const CDS_IO__CHANNEL_INFO& io_info, CString& read_data)
{
	...
	if((siCH__CTRL_MODE_HEXA_CH->Check__VARIABLE_NAME(var_name) > 0)
	|| (siCH__CAPACITOR_POS_HEXA_CH->Check__VARIABLE_NAME(var_name) > 0)
	|| (siCH__REPORT_AUXILIARY_CAP_PARM->Check__VARIABLE_NAME(var_name) > 0))
	{
		...
		if(siCH__CTRL_MODE_HEXA_CH->Check__VARIABLE_NAME(var_name) > 0)
		{
			...
			if(siCH__CTRL_MODE_HEXA_CH->Check__VARIABLE_NAME(var_name) > 0)			para_id = 1;
			else																	return -1;

			s_data[0] = 0x0ff & para_id;
			s_data[1] = 0x00;
		}
		else if(siCH__CAPACITOR_POS_HEXA_CH->Check__VARIABLE_NAME(var_name) > 0)
		{
			cmmd_id = _DRV_CMMD__REPORT_CAPACITOR_POS; // 180
			s_data_len = 2;
			r_data_len = 6;

			if(siCH__CAPACITOR_POS_HEXA_CH -> Check__VARIABLE_NAME(var_name) > 0)		para_id = 1;
			else																		return -1;
		...
		if(r_len >= r_data_len)
		{
			if(cmmd_id == _DRV_CMMD__REPORT_CONTROL_MODE)
			{
				...
				sCH__INFO_CTRL_MODE_CH->Set__DATA(str_mode);
			...
			else if(cmmd_id == _DRV_CMMD__REPORT_CAPACITOR_POS)
			{
			    ...
			    if(para_id == 1)
				{
					aCH__INFO_LOAD_POS_CH->Set__VALUE((double) val__pos_load / 100.0);
					aCH__INFO_TUNE_POS_CH->Set__VALUE((double) val__pos_tune / 100.0);
				}
				//
	...
	|| (siCH__REPORT_OUTPUT_SNS2_DATA->Check__VARIABLE_NAME(var_name) > 0))
	{
		...
		if(r_len >= r_data_len)
		{
			...
			if(sub_cmmd == 201) //Report_Sensor_Input_OutPut
			{
				...
				if(para01 == 1)
				{
					return_data = _Convert_Tofloat(4,7,buffer_arry);
					aCH__INFO_GAMMA_IN_SNS->Set__VALUE(return_data); // Gamma = Root(sData/1024)

					return_data = _Convert_Tofloat(12,15,buffer_arry);
					aCH__INFO_REAL_IN_SNS->Set__VALUE(return_data);

					return_data = _Convert_Tofloat(16,19,buffer_arry);
					aCH__INFO_IMAGE_IN_SNS->Set__VALUE(return_data);

					return_data = _Convert_Tofloat(20,23,buffer_arry);
					aCH__INFO_VOLTAGE_IN_SNS->Set__VALUE(return_data);

					return_data = _Convert_Tofloat(24,27,buffer_arry);
					aCH__INFO_CURRENT_IN_SNS->Set__VALUE(return_data);
				...
				{
					return_data = _Convert_Tofloat(4,7,buffer_arry);
					aCH__INFO_PHASE_OUT_SNS1->Set__VALUE(return_data);

					return_data = _Convert_Tofloat(8,11,buffer_arry);
					aCH__INFO_DELIVERY_POWER_OUT_SNS1->Set__VALUE(return_data);

					return_data = _Convert_Tofloat(12,15,buffer_arry);
					aCH__INFO_REAL_OUT_SNS1->Set__VALUE(return_data);

					return_data = _Convert_Tofloat(16,19,buffer_arry);
					aCH__INFO_IMAGE_OUT_SNS1->Set__VALUE(return_data);

					return_data = _Convert_Tofloat(20,23,buffer_arry);
					aCH__INFO_VOLTAGE_OUT_SNS1->Set__VALUE(return_data);

					return_data = _Convert_Tofloat(24,27,buffer_arry);
					aCH__INFO_CURRENT_OUT_SNS1->Set__VALUE(return_data);
				...
				{
					return_data = _Convert_Tofloat(4,7,buffer_arry);
					aCH__INFO_PHASE_OUT_SNS2->Set__VALUE(return_data);

					return_data = _Convert_Tofloat(8,11,buffer_arry);
					aCH__INFO_DELIVERY_POWER_OUT_SNS2->Set__VALUE(return_data);

					return_data = _Convert_Tofloat(12,15,buffer_arry);
					aCH__INFO_REAL_OUT_SNS2->Set__VALUE(return_data);

					return_data = _Convert_Tofloat(16,19,buffer_arry);
					aCH__INFO_IMAGE_OUT_SNS2->Set__VALUE(return_data);

					return_data = _Convert_Tofloat(20,23,buffer_arry);
					aCH__INFO_VOLTAGE_OUT_SNS2->Set__VALUE(return_data);

					return_data = _Convert_Tofloat(24,27,buffer_arry);
					aCH__INFO_CURRENT_OUT_SNS2->Set__VALUE(return_data);
	...
	if(siCH__REPORT_CONDENSED_MATCH_SNAPSHOT -> Check__VARIABLE_NAME(var_name) > 0 )
	{
		...
		if(r_len >= r_data_len)
		{
			...
			aCH__INFO_LOAD_POS_CH->Set__VALUE((double) return_data / 100.0);
			//Matcj1 Tune
			return_data = _Convert_ToInt(6,7,buffer_arry);
			aCH__INFO_TUNE_POS_CH->Set__VALUE((double) return_data / 100.0);
			//Match1 R
			return_data = _Convert_ToInt(8,9,buffer_arry);
			aCH__INFO_REAL_IN_SNS->Set__VALUE((double) return_data / 100.0);
			//Match1 X
			return_data = _Convert_ToInt(10,11,buffer_arry);
			aCH__INFO_IMAGE_IN_SNS->Set__VALUE((double) return_data / 100.0);

			
			//...
			//Aux CAP Position
			return_data = _Convert_ToInt(28,29,buffer_arry);
			aCH__INFO_CAP_POS_CH->Set__VALUE((double) return_data / 100.0);
			
			
			//Aux CAP Current (Primary_Sensor) 
			return_data = _Convert_ToInt(30,31,buffer_arry);
			aCH__INFO_CURRENT_OUT_SNS1->Set__VALUE((double) return_data / 100.0);


			//Aux CAP Current (Second_Sensor), For Source
			return_data = _Convert_ToInt(32,33,buffer_arry);
			aCH__INFO_CURRENT_OUT_SNS2->Set__VALUE((double) return_data / 100.0);

			//Aux CAP Voltage
			return_data = _Convert_ToInt(36,37,buffer_arry);
			aCH__INFO_VOLTAGE_OUT_SNS1->Set__VALUE((double) return_data / 100.0);

			//Aux CAP Voltage (Second_Sensor), For Source
			return_data = _Convert_ToInt(36,37,buffer_arry);
			aCH__INFO_VOLTAGE_OUT_SNS2->Set__VALUE((double) return_data / 100.0);
			
			//...
			//Frequency1 R
			return_data = _Convert_ToInt(40,43,buffer_arry);
			aCH__INFO_REAL_OUT_SNS1->Set__VALUE((double) return_data / 100.0);

			//Frequency1 X
			return_data = _Convert_ToInt(44,47,buffer_arry);
			aCH__INFO_IMAGE_OUT_SNS1->Set__VALUE((double) return_data / 100.0);
			...
			aCH__INFO_PHASE_OUT_SNS1->Set__VALUE((double) return_data / 100.0);

			//Frequency1 Power Delivery
			return_data = _Convert_ToInt(60,63,buffer_arry);
			aCH__INFO_DELIVERY_POWER_OUT_SNS1->Set__VALUE((double) return_data / 100.0);


			//... Sensor2가 Fequencey 2로 Out되는지 확인 필요한 구간
			
			//Frequency2 R
			if(dCH__MAT_TYPE ->Check__DATA(STR__SOURCE))
			{
				return_data = _Convert_ToInt(64,67,buffer_arry);
				aCH__INFO_REAL_OUT_SNS2 ->Set__VALUE((double) return_data / 100.0);

				//Frequency2 X
				return_data = _Convert_ToInt(68,71,buffer_arry);
				aCH__INFO_IMAGE_OUT_SNS2 ->Set__VALUE((double) return_data / 100.0);
				//Frequency2 Voltage
				...
				aCH__INFO_PHASE_OUT_SNS2 ->Set__VALUE((double) return_data / 100.0);
				//Frequency2 Power Delivery
				return_data = _Convert_ToInt(84,87,buffer_arry);
				aCH__INFO_DELIVERY_POWER_OUT_SNS2 ->Set__VALUE((double) return_data / 100.0);
			}
...
::__Write__DIGITAL(const CString& var_name, const CDS_IO__CHANNEL_INFO& io_info, const CString& set_data,const int item_index)
{
	...
	if(doCH__CAPACITOR_LIMIT_CH->Check__VARIABLE_NAME(var_name) > 0)
	
	{
		byte cmmd_id = _DRV_CMMD__SET_CAPACITOR_LIMIT; // Load & Tune Min/Max Set (NU)
		byte s_data_len = 10;
		byte r_data_len = 1;
		...
		{
			para_id = 1;
			para_load_min = ((int) aCH__CFG_LOAD_MIN_CH->Get__VALUE()*100);
			para_load_max = ((int) aCH__CFG_LOAD_MAX_CH->Get__VALUE()*100);
			para_tune_min = ((int) aCH__CFG_TUNE_MIN_CH->Get__VALUE()*100);
			para_tune_max = ((int) aCH__CFG_TUNE_MAX_CH->Get__VALUE()*100);
		}
		else
		{
			return -1;
	...
	if((doCH__CONTROL_MODE_CH->Check__VARIABLE_NAME(var_name)  > 0)
	|| (doCH__ACTIVE_PRESET_CH->Check__VARIABLE_NAME(var_name) > 0)
	|| (doCH__INTERNAL_EXTERNAL_PRESET_CH->Check__VARIABLE_NAME(var_name) > 0))
	{
		byte cmmd_id = 0;
		byte s_data_len = 4;
		byte r_data_len = 1;
		...
		if(doCH__CONTROL_MODE_CH->Check__VARIABLE_NAME(var_name) > 0)
		{
			cmmd_id = _DRV_CMMD__SET_CONTROL_MODE; //93
				 ...
				 if(doCH__CONTROL_MODE_CH->Check__VARIABLE_NAME(var_name) > 0)			para_id = 1;
		...
		else if(doCH__ACTIVE_PRESET_CH->Check__VARIABLE_NAME(var_name) > 0)

		{
			cmmd_id = _DRV_CMMD__SET_ACTIVE_PRESET;  // NU

			if(doCH__ACTIVE_PRESET_CH->Check__VARIABLE_NAME(var_name) > 0)
			{
				para_id = 1;
				para_set = (int) aCH__PARA_ACTIVE_PRESET_CH->Get__VALUE();
			}
		}
		else if(doCH__INTERNAL_EXTERNAL_PRESET_CH->Check__VARIABLE_NAME(var_name) > 0)
		{
	...
	if((doCH__LOAD_POS_CH->Check__VARIABLE_NAME(var_name) > 0)
	|| (doCH__TUNE_POS_CH->Check__VARIABLE_NAME(var_name) > 0))
	{
		byte cmmd_id = 0;
		byte s_data_len = 4;
		...
		if(doCH__LOAD_POS_CH->Check__VARIABLE_NAME(var_name) > 0)
		{
			...
			para_id = 1;
			para_pos = (int) (100.0 * aCH__PARA_LOAD_POS_CH->Get__VALUE());
			ch_data.Format("%.2lf", (double)(para_pos/100.0));
			sCH__CUR_LOAD_POS_CH->Set__DATA(ch_data);


		}
		else if(doCH__TUNE_POS_CH->Check__VARIABLE_NAME(var_name) > 0)
		{
			cmmd_id = _DRV_CMMD__SET_TUNE_POS;
			para_id = 1;
			para_pos = (int) (100.0 * aCH__PARA_TUNE_POS_CH->Get__VALUE());
			ch_data.Format("%.2lf", (double)(para_pos/100.0));
	...
	if(doCH__CAPACITOR_MOTOR_POS_CH->Check__VARIABLE_NAME(var_name) > 0)
	{
		...
		{
			para_id = 1;
			para_load_pos = (int) (100.0 * aCH__PARA_LOAD_POS_CH->Get__VALUE());
			para_tune_pos = (int) (100.0 * aCH__PARA_TUNE_POS_CH->Get__VALUE());

			ch_data.Format("%.2lf", (double)(para_load_pos/100.0));
			sCH__CUR_LOAD_POS_CH->Set__DATA(ch_data);

			ch_data.Format("%.2lf", (double)(para_tune_pos/100.0));
			sCH__CUR_TUNE_POS_CH->Set__DATA(ch_data);
		}
		else
		{
	...
	if(doCH__AUXILIARY_CAP_POS -> Check__VARIABLE_NAME(var_name) > 0)
	{
		...
		ch_data.Format("%.2lf", (double)(para_cap_pos/100.0));
