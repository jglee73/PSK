...
float CObj__NAVII_SERIAL
::_Convert_Tofloat(int start_idx, int end_idx, unsigned char *data)
{
	float fResult = 0.0;
	int iByte_Pos = end_idx-start_idx;
	for(int i = end_idx; i >= start_idx; i--)
	{
		*((unsigned char*)(&fResult) + iByte_Pos) = data[i];
		iByte_Pos--;
	}
	return fResult;
}
