...
LOOP_RETRY:
	...
	*/
	...
	// if(iActive__SIM_MODE > 0)
	if(dCH__CFG_CHECKLOAD_BASE_MAP->Check__DATA("DISABLE") > 0 ) // || iActive__SIM_MODE > 0 UI기준으로 Update확인
	{
	...
	else // CheckLoad 기능 우선으로 Update 확인
	{
		...
		if(bDisMatch_flag == true) // If dismatch Wafer Information
		{
			...
			if(r_act.CompareNoCase("ABORT") == 0) // ABORT를 누르면 UI기준으로 Wafer 상태 Update
			{
				if(dCH__OTR_OUT_MON__ARM_A_MATERIAL_STATUS->Check__DATA(STR__NONE) > 0)
					dCH__DRV_INFO_ARM_A_MAT_READ->Set__DATA("Absent");
				else
