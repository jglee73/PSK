					  ...
					  const CString& stn_name) 
{
	...
	*/
	...
	if(stn_name.CompareNoCase("PM2")== 0)
	{
		if(dCH__OTR_IN_PARA__ARM_TYPE->Check__DATA("A") > 0)
		{
			sDA_PM2_CHART_ARM_A_ROFFSET_DISPLAY->Set__DATA(str_r_val);
			sDA_PM2_CHART_ARM_A_TOFFSET_DISPLAY->Set__DATA(str_t_val);
			dDA_PM2_CHART_ARM_A_UPDATE_FLAG->Set__DATA("ON");		// ON 시키면 display 된다.
	
			path = m_szRoot+"\\EQP_LOG\\DATALOG\\DA_Data\\"+file_time;
			
			CreateDirectory(path, NULL);
			data_log.Format("A/%s/%s ", str_r_val, str_t_val);
			sprintf(szPath, "%s\\EQP_LOG\\DATALOG\\DA_Data\\%s\\PM2.TXT", m_szRoot, file_time);
		...
		{
			sDA_PM2_CHART_ARM_B_ROFFSET_DISPLAY->Set__DATA(str_r_val);
			sDA_PM2_CHART_ARM_B_TOFFSET_DISPLAY->Set__DATA(str_t_val);
			dDA_PM2_CHART_ARM_B_UPDATE_FLAG->Set__DATA("ON");
	...
	else if(stn_name.CompareNoCase("PM3")== 0)
	{
		if(dCH__OTR_IN_PARA__ARM_TYPE->Check__DATA("A") > 0)
		{
			sDA_PM3_CHART_ARM_A_ROFFSET_DISPLAY->Set__DATA(str_r_val);
			sDA_PM3_CHART_ARM_A_TOFFSET_DISPLAY->Set__DATA(str_t_val);
			dDA_PM3_CHART_ARM_A_UPDATE_FLAG->Set__DATA("ON");		// ON 시키면 display 된다.
			
			path = m_szRoot+"\\EQP_LOG\\DATALOG\\DA_Data\\"+file_time;
			
			CreateDirectory(path, NULL);
			data_log.Format("A/%s/%s ", str_r_val, str_t_val);
			sprintf(szPath, "%s\\EQP_LOG\\DATALOG\\DA_Data\\%s\\PM3.TXT", m_szRoot, file_time);
			FILE * fp = fopen(szPath, "a");
		...
		{
			sDA_PM3_CHART_ARM_B_ROFFSET_DISPLAY->Set__DATA(str_r_val);
			sDA_PM3_CHART_ARM_B_TOFFSET_DISPLAY->Set__DATA(str_t_val);
			dDA_PM3_CHART_ARM_B_UPDATE_FLAG->Set__DATA("ON");
	...
	else if(stn_name.CompareNoCase("PM4")== 0)
	{
		if(dCH__OTR_IN_PARA__ARM_TYPE->Check__DATA("A") > 0)
		{
			sDA_PM4_CHART_ARM_A_ROFFSET_DISPLAY->Set__DATA(str_r_val);
			sDA_PM4_CHART_ARM_A_TOFFSET_DISPLAY->Set__DATA(str_t_val);
			dDA_PM4_CHART_ARM_A_UPDATE_FLAG->Set__DATA("ON");		// ON 시키면 display 된다.

			path = m_szRoot+"\\EQP_LOG\\DATALOG\\DA_Data\\"+file_time;
			
			CreateDirectory(path, NULL);
			data_log.Format("A/%s/%s ", str_r_val, str_t_val);
			sprintf(szPath, "%s\\EQP_LOG\\DATALOG\\DA_Data\\%s\\PM4.TXT", m_szRoot, file_time);
			FILE * fp = fopen(szPath, "a");
		...
		{
			sDA_PM4_CHART_ARM_B_ROFFSET_DISPLAY->Set__DATA(str_r_val);
			sDA_PM4_CHART_ARM_B_TOFFSET_DISPLAY->Set__DATA(str_t_val);
			dDA_PM4_CHART_ARM_B_UPDATE_FLAG->Set__DATA("ON");
	...
	else if(stn_name.CompareNoCase("LBB")== 0)
	{	
		if(dCH__OTR_IN_PARA__ARM_TYPE->Check__DATA("A") > 0)
		{
			sDA_LBB_CHART_ARM_A_ROFFSET_DISPLAY->Set__DATA(str_r_val);
			sDA_LBB_CHART_ARM_A_TOFFSET_DISPLAY->Set__DATA(str_t_val);
			dDA_LBB_CHART_ARM_A_UPDATE_FLAG->Set__DATA("ON");		// ON 시키면 display 된다.

			path = m_szRoot+"\\EQP_LOG\\DATALOG\\DA_Data\\"+file_time;
			
			CreateDirectory(path, NULL);
			data_log.Format("A/%s/%s ", str_r_val, str_t_val);
			sprintf(szPath, "%s\\EQP_LOG\\DATALOG\\DA_Data\\%s\\LLB.TXT", m_szRoot, file_time);
			FILE * fp = fopen(szPath, "a");
		...
		{
			sDA_LBB_CHART_ARM_B_ROFFSET_DISPLAY->Set__DATA(str_r_val);
			sDA_LBB_CHART_ARM_B_TOFFSET_DISPLAY->Set__DATA(str_t_val);
			dDA_LBB_CHART_ARM_B_UPDATE_FLAG->Set__DATA("ON");
	...
	else if(stn_name.CompareNoCase("LBA")== 0)
	{
		if(dCH__OTR_IN_PARA__ARM_TYPE->Check__DATA("A") > 0)
		{
			sDA_LBA_CHART_ARM_A_ROFFSET_DISPLAY->Set__DATA(str_r_val);
			sDA_LBA_CHART_ARM_A_TOFFSET_DISPLAY->Set__DATA(str_t_val);
			dDA_LBA_CHART_ARM_A_UPDATE_FLAG->Set__DATA("ON");		// ON 시키면 display 된다.

			path = m_szRoot+"\\EQP_LOG\\DATALOG\\DA_Data\\"+file_time;
			
			CreateDirectory(path, NULL);
			data_log.Format("A/%s/%s ", str_r_val, str_t_val);
			sprintf(szPath, "%s\\EQP_LOG\\DATALOG\\DA_Data\\%s\\LLA.TXT", m_szRoot, file_time);
			FILE * fp = fopen(szPath, "a");
		...
		{
			sDA_LBA_CHART_ARM_B_ROFFSET_DISPLAY->Set__DATA(str_r_val);
			sDA_LBA_CHART_ARM_B_TOFFSET_DISPLAY->Set__DATA(str_t_val);
			dDA_LBA_CHART_ARM_B_UPDATE_FLAG->Set__DATA("ON");

			path = m_szRoot+"\\EQP_LOG\\DATALOG\\DA_Data\\"+file_time;
			CreateDirectory(path, NULL);
			
			data_log.Format("B/%s/%s ", str_r_val, str_t_val);
			sprintf(szPath, "%s\\EQP_LOG\\DATALOG\\DA_Data\\%s\\PM2.LLA", m_szRoot , file_time);
			FILE * fp = fopen(szPath, "a");
					    ...
					    const CString& stn_name) 
{
	...
	if(stn_name.CompareNoCase("PM1")== 0)
	{
		if(dCH__OTR_IN_PARA__ARM_TYPE->Check__DATA("A") > 0)
		{
			sDA_PM1_CHART_ARM_A_ROFFSET_DISPLAY->Set__DATA(str_r_val);
			sDA_PM1_CHART_ARM_A_TOFFSET_DISPLAY->Set__DATA(str_t_val);	
			data_log.Format("A/%s/%s ", str_t_val, str_r_val);
		}
		else
		{
			sDA_PM1_CHART_ARM_B_ROFFSET_DISPLAY->Set__DATA(str_r_val);
			sDA_PM1_CHART_ARM_B_TOFFSET_DISPLAY->Set__DATA(str_t_val);
			data_log.Format("B/%s/%s ", str_t_val, str_r_val);	
		}

		/// 파일에 남김.
		sprintf(szPath, "%s\\EQP_LOG\\DATALOG\\DA_Data\\%s\\PM1.TXT", m_szRoot , file_time);
		FILE * fp = fopen(szPath, "a");
		...
		if (dCH__CFG_PMx_DYNAMIC_ALIGN_USE[0]->Check__DATA("YES") > 0)
		{
			sDA_PM1_CHART_ROFFSET_DISPLAY->Set__DATA(str_r_val);
			sDA_PM1_CHART_TOFFSET_DISPLAY->Set__DATA(str_t_val);
		}
		else
		{
			str_r_val = "0";
	...
	else if(stn_name.CompareNoCase("PM2")== 0)
	{
		if(dCH__OTR_IN_PARA__ARM_TYPE->Check__DATA("A") > 0)
		{
			sDA_PM2_CHART_ARM_A_ROFFSET_DISPLAY->Set__DATA(str_r_val);
		...
		if (dCH__CFG_PMx_DYNAMIC_ALIGN_USE[1]->Check__DATA("YES") > 0)
		{
			sDA_PM2_CHART_ROFFSET_DISPLAY->Set__DATA(str_r_val);
			sDA_PM2_CHART_TOFFSET_DISPLAY->Set__DATA(str_t_val);
		}		
		else
		{
			str_r_val = "0";
	...
	else if(stn_name.CompareNoCase("PM3")== 0)
	{
		...
		if (dCH__CFG_PMx_DYNAMIC_ALIGN_USE[2]->Check__DATA("YES") > 0)
		{
			sDA_PM3_CHART_ROFFSET_DISPLAY->Set__DATA(str_r_val);
			sDA_PM3_CHART_TOFFSET_DISPLAY->Set__DATA(str_t_val);
		}
		else
		{
			str_r_val = "0";
	...
	else if(stn_name.CompareNoCase("PM4")== 0)
	{
		...
		if (dCH__CFG_PMx_DYNAMIC_ALIGN_USE[3]->Check__DATA("YES") > 0)
		{
			sDA_PM4_CHART_ROFFSET_DISPLAY->Set__DATA(str_r_val);
			sDA_PM4_CHART_TOFFSET_DISPLAY->Set__DATA(str_t_val);
		}
		else
		{
			str_r_val = "0";
	...
	{	
		...
		//LL은 배열로 선언 안됨. 
		if (dCH__CFG_LL2_DYNAMIC_ALIGN_USE->Check__DATA("YES") > 0)
		{
			sDA_LBB_CHART_ROFFSET_DISPLAY->Set__DATA(str_r_val);
			sDA_LBB_CHART_TOFFSET_DISPLAY->Set__DATA(str_t_val);
		}
		else
		{
			str_r_val = "0";
	...
	else if(stn_name.CompareNoCase("LBA")== 0)
	{
		...
		//LL은 배열로 선언 안됨.
		if (dCH__CFG_LL1_DYNAMIC_ALIGN_USE->Check__DATA("YES") > 0)
		{
			sDA_LBA_CHART_ROFFSET_DISPLAY->Set__DATA(str_r_val);
			sDA_LBA_CHART_TOFFSET_DISPLAY->Set__DATA(str_t_val);
		}
		else
		{
			str_r_val = "0";
