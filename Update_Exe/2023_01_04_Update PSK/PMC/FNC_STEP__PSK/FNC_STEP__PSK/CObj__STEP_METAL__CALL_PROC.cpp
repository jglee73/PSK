...
::Call__PROC(CII_OBJECT__VARIABLE* p_variable, CII_OBJECT__ALARM* p_alarm)
{
	...
	CString rcp__apc_pressure_mtorr;
	...
	// RCP ...
	{
		...
		rcp__apc_pressure_mtorr = aCH__RCP_APC_PRESSURE_mTORR->Get__STRING();
	...
	// APC.CTRL ...
	{
		...
		{
			obj_mode  = _APC_CMD__PRESSURE_mTORR;
			para_data = rcp__apc_pressure_mtorr;
		}
		...
		{
			{
				...
				obj_mode= _MAT_CMD__MANUAL_MODE;
				...
				obj_mode= _MAT_CMD__PROC_CTRL;
				MAT_SRC_OBJ__Start_MODE(obj_mode);
			}

			{ //.. Auto Set
				obj_mode= _MAT_CMD__AUTO_MODE;
				MAT_SRC_OBJ__Start_MODE(obj_mode);
			}
		}
		// BIAS ...
	...
	while(1)
	{
		...
		else if(active__fast_vac)
		{
			if(cur_sec >= cfg__stable_min_sec)
			{
				double cur__pressure_mtorr = aEXT_CH__APC_OBJ__PARA_PRESSURE_mTORR->Get__VALUE();
				double ref__pressure_mtorr = atof(rcp__apc_pressure_mtorr);

				if(cur__pressure_mtorr <= ref__pressure_mtorr)
				{
					break;
				}
			}
