		   ...
		   const int chuck_mode)
{
	...
	|| (chuck_mode == 1))
	{
		...
		// HV On ...
		{
			...
			doEXT_CH__ESC_VOLTAGE_ON->Set__DATA(STR__On);
			 ...
			 const int dechuck_mode)
{
	...
	// He Dump ...
	{
		...
		if(bActive__CENTER_USE)
		{
			...
			{
				doEXT_CH__He_Final_Out_Vlv_CENTER->Set__DATA(STR__Close);
				if(bActive__He_Final_Dump_Vlv_CENTER) // KMS :221115
				{
					if(dCH__CFG_3WAY_VLV_CLOSE_DURING_DECHUCKING->Check__DATA(STR__NO) < 0) doEXT_CH__He_Final_Dump_Vlv_CENTER->Set__DATA(STR__Open);
					else  doEXT_CH__He_Final_Dump_Vlv_CENTER->Set__DATA(STR__Close);
				}
				if(bActive__He_Side_Dump_Vlv_CENTER)			doEXT_CH__He_Side_Dump_Vlv_CENTER->Set__DATA(STR__Close);
			}
			else if(iDATA__HE_LINE_TYPE == _HE_LINE_TYPE__ICD)
	...
	// HV : STEPx Control ...
	{
		...
		for(i=0; i<i_limit; i++)
		{
			...
			// ...
			{
				...
				doEXT_CH__ESC_All_Voltage->Set__DATA(STR__On);
				doEXT_CH__ESC_VOLTAGE_ON->Set__DATA(STR__On); // KMS
	...
	// ...
	{
		...
		doEXT_CH__ESC_VOLTAGE_ON->Set__DATA(STR__Off); //KMS Voltage OFF
		doEXT_CH__ESC_All_Voltage->Set__DATA(STR__Off);
	}
	...
	if(dechuck_mode == 0)
	{
		...
		// He Dump ...
		{
			...
			if(bActive__CENTER_USE)
			{
				...
				{
					doEXT_CH__He_Final_Out_Vlv_CENTER->Set__DATA(STR__Close);
					if(bActive__He_Final_Dump_Vlv_CENTER)			doEXT_CH__He_Final_Dump_Vlv_CENTER->Set__DATA(STR__Close); // KMS Open -> Close

					if(bActive__He_Side_Dump_Vlv_CENTER)			doEXT_CH__He_Side_Dump_Vlv_CENTER->Set__DATA(STR__Close);

					if(bActive__He_Side_Exhaust_Vlv_CENTER)			doEXT_CH__He_Side_Exhaust_Vlv_CENTER->Set__DATA(STR__Close); // KMS Add Line
				}
				else if(iDATA__HE_LINE_TYPE == _HE_LINE_TYPE__ICD)
				{
					doEXT_CH__He_Final_Out_Vlv_CENTER->Set__DATA(STR__Close);
			   ...
			   const CString err_msg)
{
	...
	// ...
	{
		...
		// ...
		{
			...
			doEXT_CH__ESC_VOLTAGE_ON->Set__DATA(STR__Off); // KMS OFF
			doEXT_CH__ESC_All_Voltage->Set__DATA(STR__Off);
		}
