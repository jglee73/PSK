...
// ...
void CDlg__Lot_Tack_Time::_Init__Proc_Chart()
{
	RECT rc;
...
void CDlg__Lot_Tack_Time::_Make__Proc_Chart()
{
	...
	int x_max = mSlot_Tack.Get__Tack_Count_Max();
	if(x_max > 0)
	{
		double* p_x_stamp = new double [x_max + 1];

		for(int i=0; i<x_max; i++)		p_x_stamp[i] = i + 1;
		int x_min = p_x_stamp[0];

		p_xy_chart->xAxis()->setDateScale(x_min, x_max);

		p_layer->setXData(DoubleArray(p_x_stamp, x_max));
	...
	if(bActive__Chart_Single)
	{
		...
		if(r_index >= 0)
		{
			...
			CLS__Slot_Tack_Info* p_slot = mSlot_Tack.Get_Addr__Slot_Tack_Info(str_id);

			if(p_slot != NULL)
			{
				int db_size = p_slot->sList__Tack_Time.GetSize();
				if(db_size > 0)
				{
					...
					{
						p_data__y[i] = (double) atof(p_slot->sList__Tack_Time[i]);
					}
	...
	else
	{
		...
		for(int y=0; y<y_limit; y++)
		{
			...
			CStringArray l_tack;			
			mSlot_Tack.Get__Tack_Time(str_id,l_tack);

			int db_size = l_tack.GetSize();
			if(db_size > 0)
			{
				double* p_data__y = new double[db_size];
				...
				for(int i=0; i<db_size; i++)
				{
					p_data__y[i] = (double) atof(l_tack[i]);
				}
...
int CDlg__Lot_Tack_Time::_getDefaultBgColor()
{
	LOGBRUSH LogBrush; 
...
void CDlg__Lot_Tack_Time::OnMouseMovePlotArea()
{
	_trackLineLegend((XYChart *) mChart_Viwer.getChart(), mChart_Viwer.getPlotAreaMouseX()); 

	mChart_Viwer.updateDisplay();
}
void CDlg__Lot_Tack_Time::_trackLineLegend(XYChart *c, int mouseX)
{
