...
#define _TBL_COL__STEP_CNT						2


void CDlg__Lot_Process
::_Make__Slot_List()
{
	...
	// ...
	{
		...
		for(i=0; i<i_limit; i++)
		{
			...
			else if(i == _TBL_COL__STEP_CNT)
			{
				l__col_name.Add("Step");
				l__col_width.Add(60);
			}
			else
			{
...
::_Add__Slot_List(const CStringArray& l_slot)
{
	...
	for(int i=0; i<i_limit; i++)
	{
		...
		for(k=0; k<k_limit; k++)
		{
			...
			else if(k == _TBL_COL__STEP_CNT)		str_item = "";
			else									break;
