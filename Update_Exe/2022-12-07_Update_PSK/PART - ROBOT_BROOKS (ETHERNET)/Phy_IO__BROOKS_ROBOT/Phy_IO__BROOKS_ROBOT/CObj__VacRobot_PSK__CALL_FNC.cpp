		     ...
		     const CString& stn_slot)
{
	...
	int r_flag = Fnc__PICK_DYNAMIC_ALIGN_N(p_variable,p_alarm, stn_name,arm_type,stn_slot,is_act_handoff);
	if(r_flag > 0)
	{
		if(iActive__SIM_MODE > 0)
		{
			if(arm_type.CompareNoCase(ARM_A) == 0)	diCH__ARM_A_MAT_READ->Set__DATA("Present");
			if(arm_type.CompareNoCase(ARM_B) == 0)	diCH__ARM_B_MAT_READ->Set__DATA("Present");
		}
	}
	return r_flag;
}
			  ...
			  const CString& stn_slot)
{
	...
	if(r_flag > 0)
	{
		if(iActive__SIM_MODE > 0)
		{
			if(arm_type.CompareNoCase(ARM_A) == 0)	diCH__ARM_A_MAT_READ->Set__DATA("Absent");
			if(arm_type.CompareNoCase(ARM_B) == 0)	diCH__ARM_B_MAT_READ->Set__DATA("Absent");
		}
	}
	return r_flag;
}
