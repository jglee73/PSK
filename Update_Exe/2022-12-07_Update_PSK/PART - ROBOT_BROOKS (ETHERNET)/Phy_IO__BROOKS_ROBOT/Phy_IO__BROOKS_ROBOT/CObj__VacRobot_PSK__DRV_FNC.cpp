...
::__Read__DIGITAL(const CString& var_name,const CDS_IO__CHANNEL_INFO& io_info, CString& read_data,int& item_index)
{
	...
	*/
	...
	if(diCH__ROBOT_STATE_READ->Check__VARIABLE_NAME(var_name) > 0)
	{
		str_cmmd = "TCL rqstate";// BUSY, ERROR, ARM STATION, ARM Position, SERVO STATUS, ARM A STATUS
		int r_flag = Proc__CMMD_RSP(var_name, str_cmmd, _TO__QUERY, TRUE, l_rsp,l_err) ;
		if(r_flag >=0 && l_rsp.GetSize() > 1)
		{
			CString Parse_data;
			CStringArray r_data_array;
			int A_limt = splitString(l_rsp[0]," ",r_data_array);
			printf("%s %d",l_rsp[0],r_data_array.GetSize());
			if(r_data_array.GetSize() >= 7)
			{
				//r_data_array.GetSize()
				Parse_data = r_data_array[0];// Busy

				Parse_data = r_data_array[1]; //Error
				aiCH__ERRID->Set__VALUE(atol(Parse_data));

				Parse_data = r_data_array[2]; // Arm Station

				Parse_data = r_data_array[3]; // Arm Position

				Parse_data = r_data_array[4]; //Servo Status

				Parse_data = r_data_array[5];//ARM A STATUS
				if(Parse_data.CompareNoCase("ON") == 0) diCH__ARM_A_MAT_READ->Set__DATA("Present");
				else diCH__ARM_A_MAT_READ->Set__DATA("Absent");
				
				Parse_data = r_data_array[6];//ARM B STATUS
				if(Parse_data.CompareNoCase("ON") == 0) diCH__ARM_B_MAT_READ->Set__DATA("Present");
				else diCH__ARM_B_MAT_READ->Set__DATA("Absent");
	...
	{
		str_cmmd = "OVERALL SPEED 0.9";// AWC AVAILABLE SPEED 0.9
		int r_flag = Proc__CMMD_RSP(var_name, str_cmmd, _TO__QUERY, TRUE, l_rsp,l_err) ;
		...
		if(l_rsp.GetSize() > 0)
		{
			...
			else if(set_data.CompareNoCase("WAF_CEN.DATA") == 0)
			{
				...
				if(active_offset)
				{
					...
					// KMS : Transform R, T -> X, Y
					
					str_value.Format("%.3f", r_value*cos(t_value)); // X
					sCH__RQSTN_SNS_X_VALUE->Set__DATA(str_value);
					str_value.Format("%.3f", r_value*sin(t_value)); // Y
					sCH__RQSTN_SNS_Y_VALUE->Set__DATA(str_value);
				...
				else
				{
					...
					sCH__RQSTN_SNS_Z_VALUE->Set__DATA("0.0");
					// Transform R, T -> X, Y
					sCH__RQSTN_SNS_X_VALUE->Set__DATA("0.0");
					sCH__RQSTN_SNS_Y_VALUE->Set__DATA("0.0");
