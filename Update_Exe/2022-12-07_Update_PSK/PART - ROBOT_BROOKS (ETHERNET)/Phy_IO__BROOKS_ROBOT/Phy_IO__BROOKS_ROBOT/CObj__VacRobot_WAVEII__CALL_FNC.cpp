		     ...
		     const CString& stn_slot)
{
	...
	// ...
	{
		...
		/*
		// Pressure Check -----
		if(dEXT_CH__CFG_TRANSFER_MODE->Check__DATA("VAC") > 0)
		{
			if(Interlock__CHECK_PRESSURE_VAC(p_alarm,stn_name,act_name) < 0)
			{
				return -1;
			}
		}
		...
		{
			if(Interlock__CHECK_PRESSURE_ATM(p_alarm,stn_name,act_name) < 0)
			{
				return -1;
			}
		}
		*/
		...
		// Valve Check -----
		if(Interlock__CHECK_VALVE(p_alarm,stn_name,act_name) < 0)
		{
			return -1;
		}
	}
			    ...
			    const CString& stn_slot)
{
	...
	dEXT_IO_CH__ARM_RNE_SNS->Set__DATA("Extend");
			  ...
			  const CString& stn_slot)
{
	...
	|| (str_data.CompareNoCase("PICK_Success")    == 0))
	{
		if(str_wfr_sts.CompareNoCase("NONE") != 0)
		{
			...
			return Fnc__PLACE_DYNAMIC_ALIGN_N(p_variable,
											  p_alarm,
											  stn_name,
											  str_before_stn,
											  arm_type,
