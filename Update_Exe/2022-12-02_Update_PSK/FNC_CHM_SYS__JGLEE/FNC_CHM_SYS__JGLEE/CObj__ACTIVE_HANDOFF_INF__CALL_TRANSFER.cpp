...
::Call__TRANSFER_READY(CII_OBJECT__VARIABLE *p_variable, CII_OBJECT__ALARM *p_alarm)
{
	...
	// OBJ.LIFT_PIN ... // KMS, PIN UP -> Door Open (H/W Interlock)
	// OBJ.LIFT_PIN ...
	...
	if(bActive__OBJ_CTRL__CHM_FNC) // KMS Add, For Slot - Gas Valve Interlock
	{
		if(dEXT_CH__LINK_CFG_TRANSFER_BALLAST_ENABLED->Check__DATA(STR__YES) > 0)
		{
			int r_flag = pOBJ_CTRL__CHM_FNC->Call__OBJECT(sLINK_MODE__BALLAST_END);
