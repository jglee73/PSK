...
LOOP_RETRY:
	...
	// TMP EXHAUST-VALVE <- CLOSE // KMS 221028(Sequence Error)
	if(bActive__OBJ_CTRL__TURBO_PUMP)
	{
		{
			log_msg = "TMP.Forline Valve <- Close";
			sCH__OBJ_MSG->Set__DATA(log_msg);
			xLOG_CTRL->WRITE__LOG(log_msg);
		}

		if(pOBJ_CTRL__TURBO_PUMP->Call__OBJECT(CMMD_TMP__EXHAUST_CLOSE) < 0)
		{
			return -51; 
		}
		Sleep(1000);
	...
	if(bActive__OBJ_CTRL__TURBO_PUMP)
	{
		...
		if(dEXT_CH__TURBO_PUMP__MON_PUMP_STATE->Check__DATA(STR__NORMAL) < 0)
		{
			...
			// Low-Vacuum Pumping ...
			{
				...
				if(cur_press > cfg_press)
				{
					int r_flag = Fnc__LOW_VAC_PUMP(p_variable,p_alarm, 1, -1);
					if(r_flag < 0)			return -10001;
