...
{	
	...
	// KMS, For Stop Ballast When Door Open
	{
		def_name = "OBJ__CHM_FNC";
		p_ext_obj_create->Get__DEF_CONST_DATA(def_name, obj_name);

		def_check = x_utility.Check__Link(obj_name);
		bActive__OBJ_CTRL__CHM_FNC = def_check;
		if(def_check)
		{
			pOBJ_CTRL__CHM_FNC = p_ext_obj_create->Create__OBJECT_CTRL(obj_name);
			//
			def_name = "BALLAST.END";
			p_ext_obj_create->Get__DEF_CONST_DATA(def_name, def_data);
			sLINK_MODE__BALLAST_END = def_data;
			{
				CString ch_name;
				def_name = "CFG_TRANSFER_BALLAST_ENABLED";
				p_ext_obj_create->Get__DEF_CONST_DATA(def_name, ch_name);
				p_ext_obj_create->Get__CHANNEL_To_OBJ_VAR(ch_name, obj_name,var_name);
				LINK__EXT_VAR_DIGITAL_CTRL(dEXT_CH__LINK_CFG_TRANSFER_BALLAST_ENABLED, obj_name,var_name);
			}
		}
	}
