...
::_Update__LPx_STATE(const int lp_id, const CString& rsp_data)
{
	...
	for(i=0; i<i_limit; i++)
	{
		...
		{
				 if(i_data == _VAL__FOUP_EXIST__NO)					str_data = STR__NONE;
			else if(i_data == _VAL__FOUP_EXIST__YES)				str_data = STR__EXIST;
			else 													str_data = STR__UNKNOWN;
...
::_Update__LPx_MAP(const int lp_id, const CString& rsp_data)
{
	...
	for(k=0; k<k_limit; k++)
	{
		...
		int i_data = atoi(ch_data);
		     ...
		     if(i_data == 0)		ch_data = "ABSENT";
		else if(i_data == 1)		ch_data = "PRESENT";	
		else if(i_data == 2)		ch_data = "DOUBLE";
		else if(i_data == 3)		ch_data = "CROSSED";
		else if(i_data == 4)		ch_data = "INVALID";
