...
void CObj__LPx_CTRL::_Update__LPx_MAP()
{
	CString log_msg;
	CString log_bff;
	...
	CString ch_data = siEXT_CH__LP_MAP_DATA->Get__STRING();
	log_bff.Format(" * %s <- %s \n", siEXT_CH__LP_MAP_DATA->Get__CHANNEL_NAME(), ch_data);
	log_msg += log_bff;

	for(int i=0; i<CFG_LPx__SLOT_SIZE; i++)
	{
		log_bff.Format("  Slot%02d <- %s \n", i+1, dEXT_CH__LPx_MAP__RESULT[i]->Get__STRING());
		log_msg += log_bff;
	}
