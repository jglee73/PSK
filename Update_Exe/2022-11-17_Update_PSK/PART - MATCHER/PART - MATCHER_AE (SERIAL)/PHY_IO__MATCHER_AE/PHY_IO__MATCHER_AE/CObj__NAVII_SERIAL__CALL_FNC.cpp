...
::Call__INIT(CII_OBJECT__VARIABLE* p_variable, CII_OBJECT__ALARM* p_alarm)
{
	...
	// Control.Mode 
	{
		...
		if(iActive_SIM > 0) sCH__INFO_CTRL_MODE_CH1->Set__DATA(STR__HOST);
	}


	{// Clear Latched Faults
		doCH__CONTROL_MODE_CH1->Set__DATA(STR__SET);
	}
...
::Call__AUTO_CH_X(CII_OBJECT__VARIABLE* p_variable,CII_OBJECT__ALARM* p_alarm, const bool active_all)
{
	...
	else
	{
		...
		{
			doCH__CONTROL_MODE_CH1->Set__DATA(STR__AUTO);
			if(iActive_SIM > 0) sCH__INFO_CTRL_MODE_CH1->Set__DATA(STR__AUTO);
		}			
		if(ch_id == 2)
		{
			if(active__use_ch2)			doCH__CONTROL_MODE_CH2->Set__DATA(STR__AUTO);
...
::Call__MANUAL_CH_X(CII_OBJECT__VARIABLE* p_variable,CII_OBJECT__ALARM* p_alarm, const bool active_all)
{
	...
	else
	{
		...
		if(ch_id == 1)
		{
			...
			if(iActive_SIM > 0)
			{
				sCH__INFO_CTRL_MODE_CH1->Set__DATA(STR__HOST);
				aCH__INFO_CAP_POS_CH -> Set__VALUE(atof(aCH__PARA_CAP_POS_CH->Get__STRING()));
				aCH__INFO_LOAD_POS_CH1 ->Set__VALUE(atof(aCH__PARA_LOAD_POS_CH1->Get__STRING()));
				aCH__INFO_TUNE_POS_CH1 ->Set__VALUE(atof(aCH__PARA_TUNE_POS_CH1->Get__STRING()));
			}
		}
		if(ch_id == 2)
		{
...
::Call__CTRL_MODE_CH_X(CII_OBJECT__VARIABLE* p_variable,CII_OBJECT__ALARM* p_alarm, const int ch_id)
{
	...
	if((ch_id == 0) || (ch_id == 1))
	{
		...
		if(iActive_SIM > 0) sCH__INFO_CTRL_MODE_CH1->Set__DATA(ch_data);
	}
...
::Call__LOAD_POS_CH_X(CII_OBJECT__VARIABLE* p_variable,CII_OBJECT__ALARM* p_alarm, const int ch_id)
{
	...
	if((ch_id == 0) || (ch_id == 1))
	{
		...
		if(iActive_SIM > 0) aCH__INFO_LOAD_POS_CH1 ->Set__VALUE(atof(aCH__PARA_LOAD_POS_CH1->Get__STRING()));
	}
...
::Call__TUNE_POS_CH_X(CII_OBJECT__VARIABLE* p_variable,CII_OBJECT__ALARM* p_alarm, const int ch_id)
{
	...
	if((ch_id == 0) || (ch_id == 1))
	{
		...
		if(iActive_SIM > 0) aCH__INFO_TUNE_POS_CH1 ->Set__VALUE(atof(aCH__PARA_TUNE_POS_CH1->Get__STRING()));
	}
