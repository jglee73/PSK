...
::Mon__MONITOR(CII_OBJECT__VARIABLE* p_variable, CII_OBJECT__ALARM* p_alarm)
{
	...
	// ...
	{
		...
		sCH__INFO_CAP_POS_CH->Set__DATA(ch_data);
	...
	while(1)
	{
		...
		{
			if(dCH__PARA_SEL_CH ->Check__DATA(STR__SOURCE)) siCH__REPORT_OUTPUT_SNS2_DATA -> Get__STRING();
		}
		...
		if(iActive_SIM > 0)
		{
			...
			// CAP.POS ...
			{
				sCH__CUR_CAP_POS_CH->Get__DATA(ch_data);
				sCH__INFO_CAP_POS_CH->Set__DATA(ch_data);
			}
		}
	}
