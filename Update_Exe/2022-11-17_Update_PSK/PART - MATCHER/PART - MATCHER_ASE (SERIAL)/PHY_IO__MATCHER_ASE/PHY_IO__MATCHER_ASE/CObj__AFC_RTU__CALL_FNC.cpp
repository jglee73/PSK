					 ...
					 const unsigned char read_num)
{
	...
	s_data[i++] = 0x0ff & (read_num >> 8);		// 4
	s_data[i++] = 0x0ff & (read_num >> 0);		// 5
					 ...
					 const int s_len)
{
	...
	if(s_cmmd == _CMMD_ID__READ_INPUT) // KMS
	{
		// Slave Address(1) + Function Code(1) + Byte Count(1) + Register Value(2*N)
		r_len = 3 + (s_reg * 2);
	}
	else if(s_cmmd == _CMMD_ID__WRITE_HOLDING) //KMS
	{
		// Slave Address(1) + Function Code(1) + Regisger Add(2) + Register Valvue(2)+ CRC(2)
		r_len = 8;  
	}
...
unsigned int CObj__AFC_RTU
::_Make__WRITE_PACKET_EX(unsigned char* s_data, 
						 const unsigned char addr_id,
						 const unsigned char cmmd_id,
						 const unsigned int  start_addr,
						 const unsigned short set_i) // KMS
{
	int i = 0;
	...
	s_data[i++] = 0x0ff & (start_addr >> 8);		// 2
	s_data[i++] = 0x0ff & (start_addr >> 0);		// 3
	s_data[i++] = 0x0ff & (set_i >> 8);				// 4
	s_data[i++] = 0x0ff & (set_i >> 0);				// 5
