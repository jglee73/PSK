...
#define MON_ID__IO_TYPE_ALARM_CHECK				5
...
int CObj__RF_STD::__DEFINE__VARIABLE_STD(p_variable)
{
	...
	// ...
	{
		...
		{
			p_variable->Add__MONITORING_PROC(1.0, MON_ID__IO_TYPE_ALARM_CHECK);
		}	
	}
	return 1;
}
int CObj__RF_STD::__DEFINE__ALARM(p_alarm)
{
	...
	{
		alarm_id = ALID__RF_DEVICE_WARNING_EXIST;
		iLIST_ALID__RF.Add(alarm_id);

		alarm_title  = title;
		alarm_title += " RF GENERATOR - GENERATOR DEVICE WARNING.";

		alarm_msg = "Generator device warning exist.";
...
int CObj__RF_STD::__CALL__CONTROL_MODE(mode,p_debug,p_variable,p_alarm)
{
	...
	if((flag < 0)||(p_variable->Check__CTRL_ABORT() > 0))
	{
		...
		// ...
		{
			...
			if(flag == -21) alm_msg +="\n Power On Fail, Please Check Matcher State.";
...
int CObj__RF_STD::__CALL__MONITORING(id,p_variable,p_alarm)
{
	...
	if(id == MON_ID__IO_TYPE_ALARM_CHECK)		Mon__IO_TYPE_ALARM_CHECK(p_variable, p_alarm);
