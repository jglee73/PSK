...
// ...
#define  MON_ID__DRV_STATE									1
#define  MON_ID__IO_MONITOR									2
#define  MON_ID__RECV_PROC									3
...
int CDriver__LPx_ETHERNET::__DEFINE__VARIABLE_STD(p_variable)
{
	...
	// ...
	{
		p_variable->Add__MONITORING_PROC(3.0, MON_ID__DRV_STATE);
...
{
	if(id == MON_ID__DRV_STATE)			Mon__DRV_STATE(p_variable,p_alarm);
	if(id == MON_ID__IO_MONITOR)		Mon__IO_MONITOR(p_variable,p_alarm);
	if(id == MON_ID__RECV_PROC)			Mon__RECV_PROC(p_variable, p_alarm);
