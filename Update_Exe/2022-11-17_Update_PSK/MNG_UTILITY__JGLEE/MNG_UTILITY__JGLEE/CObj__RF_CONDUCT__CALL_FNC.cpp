					...
					const int db_index)
{
	...
	// 1. RF Ctrl
	{
		...
		{
			str_msg = "RF Control - Power Set Aborted ...";
			xCH__OBJ_MSG->Set__DATA(str_msg);
		...
		if(p_rf_ctrl->Call__OBJECT(CMMD_MODE__RF__ON_POWER) < 0)
		{
			str_msg = "RF Control - Power ON Aborted ...";
			xCH__OBJ_MSG->Set__DATA(str_msg);
	...
	p_rf_ctrl->Call__OBJECT(CMMD_MODE__RF__OFF); // KMS : Power OFF
