...
::Get__CFG_STN_NUM(const CString& stn_name, const int slot_id)
{
	CString cfg_stn_num = "";
	...
	if(ll_i >= 0)
	{
		...
		if(slot_index >= CFG_LLx__SLOT_SIZE)		return "";

		aCH__OTR_IN_CFG__LLx_STN_NUM_X[ll_i][slot_index]->Get__DATA(cfg_stn_num);
	}
...
CString  CObj_NET__VAC_PERSIMMON
::Get__CFG_STN_CPTR_NUM(const CString& stn_name, const int slot_id)
{
	CString cfg_val;
	...
	// LLx ...
	{
		...
		if(ll_i >= 0)
		{
			...
			aCH__OTR_IN_CFG_LLx_CPTR_SNS_NUM_X[ll_i][slot_index]->Get__DATA(cfg_val);
		}
	}
	// PMx ...
	{
