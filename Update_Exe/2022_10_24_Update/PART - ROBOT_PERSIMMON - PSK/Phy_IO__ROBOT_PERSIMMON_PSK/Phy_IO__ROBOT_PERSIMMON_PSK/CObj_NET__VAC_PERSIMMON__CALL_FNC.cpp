				 ...
				 CII_OBJECT__ALARM* p_alarm)
{
	// Set.Speed ...
	{
		doCH__COMMAND->Set__DATA(_Cmmd__Set_Speed);

		if(iSim_Flag > 0)		
		{
			CString ch_data = aCH__CFG_ROBOT_SPEED_PERCENT->Get__STRING();
			sCH__DRV_INFO_ROBOT_SPEED->Set__DATA(ch_data);
		}
	}
	return 1;
}
		    ...
		    const CString& stn_slot)
{
	...
	{
		if((iSim_Flag > 0)
		|| (dCH__CFG_ROBOT_DRV_WAFER_CHECK->Check__DATA(STR__NO) > 0))
		{
			 ...
			 const CString& stn_slot)
{
	...
	// Set Station
	{
		str__trg_stn_num = Get__CFG_STN_NUM(stn_name, slot_id);
		if(str__trg_stn_num.GetLength() < 1)			return -111;
			  ...
			  const CString& stn_slot)
{
	...
	// Set Station ...
	{
		...
		CString str__trg_stn_num = Get__CFG_STN_NUM(stn_name, slot_id);
		if(str__trg_stn_num.GetLength() < 1)			return -111;
			   ...
			   const CString& stn_slot)
{
	...
	{
		if(dCH__MON_ARM_A_MATERIAL_STATUS->Check__DATA(STR__NONE) > 0)		active_pick = true;
	}
	else if(arm_type.CompareNoCase(ARM_B) == 0)
	{
		if(dCH__MON_ARM_B_MATERIAL_STATUS->Check__DATA(STR__NONE) > 0)		active_pick = true;
	}
	else
	{
		return -11;
			  ...
			  const CString& stn_slot)
{
	...
	// ...
	{
		...
		if(arm_type.CompareNoCase(ARM_A) == 0)
		{
			if(dCH__MON_ARM_A_MATERIAL_STATUS->Check__DATA(STR__NONE) > 0)			str__zpos = "Down";
			else																	str__zpos = "Up";
		}
		else if(arm_type.CompareNoCase(ARM_B) == 0)
		{
			if(dCH__MON_ARM_B_MATERIAL_STATUS->Check__DATA(STR__NONE) > 0)			str__zpos = "Down";
			else																	str__zpos = "Up";
		}
...
::Fnc__HOME(CII_OBJECT__VARIABLE* p_variable,CII_OBJECT__ALARM* p_alarm)
{
	...
	|| (dCH__CFG_ROBOT_DRV_WAFER_CHECK->Check__DATA(STR__NO) > 0))
	{
		if(dCH__MON_ARM_A_MATERIAL_STATUS->Check__DATA(STR__NONE) > 0)			sCH__MON_ROBOT_ARM_A_LOAD->Set__DATA(STR__OFF);
		else																	sCH__MON_ROBOT_ARM_A_LOAD->Set__DATA(STR__ON);

		if(dCH__MON_ARM_B_MATERIAL_STATUS->Check__DATA(STR__NONE) > 0)			sCH__MON_ROBOT_ARM_B_LOAD->Set__DATA(STR__OFF);
		else																	sCH__MON_ROBOT_ARM_B_LOAD->Set__DATA(STR__ON);
	}

	if(Call__SET_PARA(p_variable, p_alarm) < 0)
	{
		return -13;
	}
	return 1;
