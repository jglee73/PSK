...
#define _RST_COL__OFF_R							10 // 3
#define _RST_COL__OFF_T							11 // 4
...
#define _RST_COL__OFF_X							3 //10
#define _RST_COL__OFF_Y							4 //11
...
::_Make__Result_List(CListCtrl *p_table)
{
	...
	// ...
	{
		...
		for(i=0; i<i_limit; i++)
		{
			...
			// KMS : 221018 X,Y Offset Ãß°¡
			else if(i == _RST_COL__OFF_X)	
			{
				l__col_name.Add("Offset_X");
				l__col_width.Add(120);
			}
			else if(i == _RST_COL__OFF_Y)
			{
				l__col_name.Add("Offset_Y");
				l__col_width.Add(120);
			}
			else
			{
					...
					const CStringArray& l_off_y)
{
	...
	for(i=0; i<i_limit; i++)
	{
		...
		for(k=0; k<k_limit; k++)
		{
			...
			else if(k == _RST_COL__OFF_X)
			{
				item__off_x = l_off_x[i];
			}
			else if(k == _RST_COL__OFF_Y)
			{
				item__off_y = l_off_y[i];
			}
			else
			{
				active__end_check = 1;
		...
		for(k=0; k<k_limit; k++)
		{
			...
			else if(k == _RST_COL__OFF_X)			str_item = item__off_x;
			else if(k == _RST_COL__OFF_Y)			str_item = item__off_y;
			else									break;
