					  ...
					  const int pmc_id)
{
	...
	if(bActive__VAC_RB__RNE_PM_X[pm_index])				diEXT_CH__VAC_RB__RNE_PM_X[pm_index]->Link__UPPER_OBJECT_ABORT(sObject_Name);
				...
				const double cfg_timeout)
{
	...
	else
	{
		...
		else
		{
			...
			if(bActive__PMx__SV_OPEN_X[pm_index])
			{
				doEXT_CH__PMx__SV_OPEN_X[pm_index]->Set__DATA(STR__OFF);

				Sleep(100);
			}
			if(bActive__PMx__SV_CLOSE_X[pm_index])
			{
				doEXT_CH__PMx__SV_CLOSE_X[pm_index]->Set__DATA(STR__ON);
			}
		}
	}
	...
	if(bActive__PMx__SV_CLOSE_X[pm_index])
	{
		doEXT_CH__PMx__SV_CLOSE_X[pm_index]->Set__DATA(STR__OFF);
	}
	return r_ret;
}
			   ...
			   const double cfg_timeout)
{
	...
	else
	{
		...
		else
		{
			...
			if(bActive__PMx__SV_OPEN_X[pm_index])
			{
				doEXT_CH__PMx__SV_OPEN_X[pm_index]->Set__DATA(STR__ON);
			}
		}
	}
